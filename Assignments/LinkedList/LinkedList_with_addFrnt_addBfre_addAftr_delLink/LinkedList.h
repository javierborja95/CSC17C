...
class LinkedList{
    ...
    public:
        ...
        //Assignment member functions
        
        void addFrnt(const T&);         //Add to beginning of list
        void addBfre(const T&, int);    //Add before link
        void addAftr(const T&, int);    //Add after link
        void delLink(const T&);         //Delete link that contains this value
};
...
//Input -> data   Data to embed at the beginning link in the list
template<class T>
void LinkedList<T>::addFrnt(const T &data){
    Link *add=new Link;        //Create the new link
    add->data=data;            //Add the data
    add->linkPtr=front;        //New link points to old front
    front=add;                 //Front is now the new link
}

//Function Add a link and data before an index
//Input -> Data to be added to a new link
//      -> A number to a index link
template<class T>
void LinkedList<T>::addBfre(const T &data, int n){
    if(n<0||n>cntLst()-1){
        cout<<"Error: Attempting to add out of range"<<endl;
        return;
    }
    else if(n==0){
        addFrnt(data);
        ...
        Link *add=new Link;        //Create the new link
        add->data=data;            //Add the data
        add->linkPtr=temp;//New link points to next pointer
        next->linkPtr=add;         //Pointer before now points to new
    }
}

//Function Add a link and data after a index
//Input -> Data to be added to a new link
//      -> A number to an index link
template<class T>
void LinkedList<T>::addAftr(const T &data, int n){
    if(n==cntLst()-1){
        addLst(data);
    }
    else addBfre(data,n+1);
}

//Function Deletes a link with matching value
//Input -> Value to match existing link
template<class T>
void LinkedList<T>::delLink(const T &data){
    if(findLst(data)==false)return; //Data not found, don't delete anything
    next->linkPtr=temp->linkPtr; //Link before value found now points to link after
    delete temp; //Link with value found is now deleted
}
